...
Веб-приложение для учёта **читателей** и **книг**: выдача и возврат, **поиск в шапке** (в разделе книг — по каталогу, в разделе читателей — по читателям), отдельные страницы **`/books/search`** и **`/people/search`**, пагинация и сортировка списка книг. У каждой книги задаётся **жанр** из фиксированного списка (**`Genre`**, JPA `ENUM` строкой); при создании и редактировании жанр выбирается в форме, в списке, поиске и карточке жанр показывается текстом. **Spring Boot 3**, **Spring Data JPA**, **Hibernate 6**, **Thymeleaf**, **PostgreSQL**; миграции **Flyway**; **Spring Security** включён с **CSRF** для форм; тесты на **JUnit 5**, **Mockito**, **H2**.
